        ...
        if ($vars['full'] == true) {
	        if (is_array($vars['config']->group) && sizeof($vars['config']->group) > 0){
		        ...
		        foreach($vars['config']->group as $shortname => $valtype) {
		          ...
		          if(is_array($valtype)){
    		        list($valtype,$options,$field_context,$field_access)=$valtype;
    		        if($field_context!=$group_context) continue;
    		        if($field_access!=ACCESS_PUBLIC) continue;
		          }
		          ...
		          if ($shortname != "name") {
						...
						echo "<b>";
						echo elgg_echo("{$field_context}:{$shortname}");
						echo ": </b>";
        		        ...
        		        if($shortname=="group_type"){
        		          echo elgg_view('output/tags', array('tags' => elgg_echo($value),'object'=>'group'));
        		        }
        		        else{
...
?>
		...
		<a href="<?php echo $vars['url']; ?>mod/groups/edit.php?group_guid=<?php echo $vars['entity']->getGUID(); ?>&group_context=<?php echo $group_context?>"><?php echo elgg_echo("edit"); ?></a>
